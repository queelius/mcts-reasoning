...
class ReasoningMCTS(MCTS):
    ...
    def with_compositional_actions(self, enabled: bool = True) -> 'ReasoningMCTS':
    ...
    def _get_compositional_actions(self, state: str) -> List['CompositionalAction']:
        """Get compositional actions."""
        # Import here to avoid circular dependency
        try:
            from .actions.compositional_actions import (
                CompositionalAction,
                CognitiveOperation,
                FocusAspect,
                ReasoningStyle,
            ...
            # Sample a subset of possible actions for efficiency
            # (30,720 total combinations is too many)
            actions = []
            
            # Strategic sampling: ensure diversity
            operations = list(CognitiveOperation)
            focuses = list(FocusAspect)
            styles = list(ReasoningStyle)
            
            # Sample 10-20 diverse actions
            for _ in range(min(15, len(operations) * 2)):
            ...
            # Fallback to simple actions if compositional not available
            return self._get_simple_actions(state)
    ...
    def _take_action(self, state: str, action: Any) -> str:
        ...
        if hasattr(action, 'to_prompt'):
            # Compositional action
            prompt = action.to_prompt(state, self.original_question)
        else:
...
"""
    ...
    def _is_terminal_state(self, state: str) -> bool:
        ...
        if not self.terminal_check_with_llm or not self.llm:
            # Simple heuristic
            terminal_phrases = [
                "final answer:",
                "conclusion:",
            ...
            return any(phrase in state_lower for phrase in terminal_phrases)
        
        # Use LLM to check
        prompt = f"""
Analyze this reasoning chain and determine if it has reached a complete solution.
...
Has this reasoning reached a complete solution? Answer YES or NO:
"""
