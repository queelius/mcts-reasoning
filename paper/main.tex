...
\author{Technical Report v0.5.2}
\date{\today}
...
In standard MCTS for games, different positions have different legal moves. Similarly, different reasoning states may have different available actions. The canonical implementation uses only \textsc{Continue}, but the \texttt{ExtendedActionSpace} class allows additional actions such as \textsc{Compress} for long traces.
\end{remark}
...
%------------------------------------------------------------------------------
\subsection{Tool-Augmented Reasoning (MCP)}
...
%------------------------------------------------------------------------------
\subsection{Path Sampling and Self-Consistency}
...
%------------------------------------------------------------------------------
\subsection{Tree Serialization and Continuation}
...
\begin{enumerate}
    ...
    \item \textbf{Practical capabilities} including tool integration, path sampling, and tree serialization
\end{enumerate}
...
Future research directions---including extended action spaces (e.g., \textsc{Compress}, \textsc{Verify}), algorithm variants (parallel MCTS, learned value functions), and graph-based reasoning structures---are discussed in a companion working paper on research directions for MCTS-based LLM reasoning.
